...
- [Testing](#testing)
- [Author](#author)
- [AELMDB storage backend](#aelmdb-storage-backend)
...
## AELMDB storage backend

The C++ storage layer includes `negentropy/storage/SliceAELMDB.h`, an optional
backend that maps a stable key-range window to AELMDB entry ranks and uses
AELMDB range hashsums for reconciliation fingerprints.  It requires
**AELMDB**, `MDB_AGG_ENTRIES`, `MDB_AGG_HASHSUM`, and key-sourced
hashes (`MDB_AGG_HASHSOURCE_FROM_KEY`) with `MDB_HASH_SIZE == ID_SIZE`
(currently 32 bytes).  The backend is exercised end-to-end by the companion
`bench-aelmdb` slice and advanced reconciliation tests.
